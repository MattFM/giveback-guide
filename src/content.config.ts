...
const projects = defineCollection({
  ...
  schema: notionPageSchema({
    properties: z.object({
      ...
      pCountry: transformedPropertySchema.multi_select.transform((value) =>
        Array.isArray(value) ? value : [value],
      ),
      pLocale: transformedPropertySchema.multi_select.transform((value) =>
        Array.isArray(value) ? value : [value],
      ),
      ...
      pImpactsNames: propertySchema.rollup
        .optional()
        .transform((value: any) => {
          // This should contain the names from the rollup property
          if (!value) return [];
          ...
          // Handle array type rollups (multiple values)
          if (
            rollupValue.type === "array" &&
            Array.isArray(rollupValue.array)
          ) {
            return rollupValue.array
              .map((item: any) => {
                // Handle title type (most common for Name fields)
      ...
      pInstagramURL: transformedPropertySchema.url,
...
const stays = defineCollection({
  ...
  schema: notionPageSchema({
    properties: z.object({
      ...
      sCountry: transformedPropertySchema.multi_select.transform((value) =>
        Array.isArray(value) ? value : [value],
      ),
      sLocale: transformedPropertySchema.multi_select.transform((value) =>
        Array.isArray(value) ? value : [value],
      ),
      // Relation properties (actual relations to other databases)
      ...
      // Rollup properties for names (what we use in UI)
      sTypesNames: propertySchema.rollup.optional().transform((value: any) => {
        // This should contain the names from the rollup property
        if (!value) return [];
      ...
      sImpactsNames: propertySchema.rollup
        .optional()
        .transform((value: any) => {
          // This should contain the names from the rollup property
          if (!value) return [];
      ...
      sFacilities: transformedPropertySchema.multi_select.transform((value) =>
        Array.isArray(value) ? value : [value],
      ),
